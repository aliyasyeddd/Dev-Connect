...
saving the user to the database
-> after saving the user to the database we will get the saved user object which will contain the _id field
-> generated by MongoDB and we can use that _id field to generate a JWT token for the user
-> why we need to generate JWT token after signup??
-> after signup the user will be automatically logged in and for that we need to generate a JWT token for the user and send it to the frontend in the cookie so that the frontend can use that token to authenticate the user in subsequent requests
-> and it redirects the user to the profile page where it will show the user's profile data and
-> for that it needs to authenticate the user using the token in the cookie
